...
node_t *new_node(long val, node_t *next) {
  node_t *node = (node_t *)P_MALLOC(sizeof(node_t));
  node->val = val;
  node->next = next;
  return node;
}
...
long set_add(TM_ARGDECL long val)
{
	...
	if (mode == 0) {
		prev = set->head;
		next = TM_SHARED_READ_P(prev->next);
		while (1) {
			v = TM_SHARED_READ(next->val);
			if (v >= val)
				break;
			prev = next;
			next = TM_SHARED_READ_P(prev->next);
		...
		res = (v != val);
		if (res) {
			TM_SHARED_WRITE_P(prev->next, new_node(val, next));
		}
	} else {
		prev = set->head;
		next = TM_SHARED_READ_P(prev->next);
...
int set_remove(TM_ARGDECL long val)
{
	...
	if (mode == 0) {
		...
		res = (v == val);
		if (res) {
			n = TM_SHARED_READ_P(next->next);
			TM_SHARED_WRITE_P(prev->next, n);
			TM_FREE(next);
		}
	} else {
		prev = set->head;
