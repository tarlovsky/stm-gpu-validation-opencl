...
# sudo bash plot-detailed.sh yada|intruder|vacation|kmeans cpu|gpu
...
echo "set bmargin 3" | tee -a $FILES
echo "set border 1 lc rgb \"#606060\" " | tee -a $FILES
...
echo "set format y \"%0.3f\"" | tee -a $FILES
...
for i in ${!benchmark_arr[@]}; do
    ...
    for j in ${!thread_count[@]}; do
        ...
        echo  "      '$RESULTS_DIR/$n-${benchmark_arr[$i]}-cluster' u 2:xtic(1) t col lc rgbcolor \"#${blue_pallet[((COUNT))]}\" lt 1 fs pattern 3, \\"  >> $FILE_time
        echo  "      ''               u (\$16>=\$2 ? \$16-\$2 : NaN) t col lc rgbcolor \"#${blue_pallet[((COUNT))]}\" fs pattern 6, \\"  >> $FILE_time
        ...
        echo  "      ''               u (\$0-1+0.26):((\$16!=0)?\$16:NaN):(sprintf('%2.6f', \$16)) notitle w labels offset first leftcolumn_offset_$n rotate by 90 left font \",8\", \\"  >> $FILE_time
        ...
        #Lables 10 val succ 12 val fail
        echo  "      ''               u (\$0-1-0.27):(\$4):(sprintf('%d', \$4)) notitle w labels offset first leftcolumn_offset_$n rotate by 90 left textcolor \"#8f8800\" font \",8\", \\"  >> $FILE_c_a
        ...
        #8-validation calls percentage compared to commits
        echo  "      ''               u (\$0-1):(\$6-\$6):(sprintf('%.g tx/s', ((\$16>0)?((\$4)/\$16):(0)))) notitle w labels offset first leftcolumn_offset_$n rotate by 90 right font \",6\", \\"  >> $FILE_c_a
        #errors
