...
//		};
	...
	protected:
        ...
        /*tarlovsky*/
        double val_time;
        unsigned long long stat_commits;
        unsigned long long stat_aborts;
        unsigned long long stat_val_reads;
        unsigned long long stat_val_succ;
        unsigned long long stat_val_fail;
...
inline void wlpdstm::TxMixinv::ThreadShutdown() {
    ...
    printf("%lf %llu %llu %llu %llu %llu\n", val_time, stat_commits, stat_aborts, stat_val_reads, stat_val_succ, stat_val_fail);
...
inline bool wlpdstm::TxMixinv::Validate() {
	...
	for(iter = read_log.begin();iter.hasNext();iter.next()) {
		...
		if(currentVersion != entry.version) {
            /*tarlovsky*/
		    TIMER_READ(stop);
            val_time += TIMER_DIFF_SECONDS(start, stop);
            stat_val_fail++;
			return false;
		}
	}
