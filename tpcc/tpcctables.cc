...
void TPCCTables::initialize(){
    items_.initialize();
    warehouses_.initialize();
    stock_.initialize();
    districts_.initialize();
    customers_.initialize();
    orders_.initialize();
    orders_by_customer_.initialize();
    orderlines_.initialize();
    orderlines_.initialize();
...
__attribute__((transaction_safe)) void TPCCTables::internalOrderStatus(TM_ARGDECL Customer* customer, OrderStatusOutput* output) {
    ...
    if(customer->c_id > 2){
        ...
        output->c_balance = TM_SHARED_READ_F(customer->c_balance);
...
bool TPCCTables::newOrder(TM_ARGDECL int64_t warehouse_id, int64_t district_id, int64_t customer_id, const std::vector<NewOrderItem>& items, const char* now, NewOrderOutput* output,TPCCUndo** undo) {

    uint64_t items_size = items.size();
    int64_t* warehouse_set = (int64_t*) TM_MALLOC(items_size * sizeof(int64_t));
    Item** item_tuples = (Item**) TM_MALLOC(items_size * sizeof(Item*));
...
__attribute__((transaction_safe)) bool TPCCTables::newOrderRemote(TM_ARGDECL int64_t home_warehouse, int64_t remote_warehouse,
    ...
    for (int i = 0; i < getVectorSize(items); ++i) {
        ...
        Stock* stock = findStock(TM_ARG item.ol_supply_w_id, item.i_id);
          ...
          if (TM_SHARED_READ(stock->s_quantity) >= item.ol_quantity + 10) {
            TM_SHARED_WRITE(stock->s_quantity, TM_SHARED_READ(stock->s_quantity)-item.ol_quantity);
          } else {
            TM_SHARED_WRITE(stock->s_quantity, TM_SHARED_READ(stock->s_quantity)-item.ol_quantity+91);
          }
            TM_SHARED_WRITE(stock->s_ytd, TM_SHARED_READ(stock->s_ytd)+item.ol_quantity);
            TM_SHARED_WRITE(stock->s_order_cnt, TM_SHARED_READ(stock->s_order_cnt)+1);
          // newOrderHome calls newOrderRemote, so this is needed
          if (item.ol_supply_w_id != home_warehouse) {
              // remote order
              TM_SHARED_WRITE(stock->s_remote_cnt, TM_SHARED_READ(stock->s_remote_cnt)+1);
          }
...
/*    __transaction_atomic { */
    ...
    Warehouse* w = findWarehouse_(warehouse_id);
    District* d  = findDistrict_(warehouse_id, district_id);
     Customer* customer  = findCustomer_(c_warehouse_id, c_district_id, customer_id);
    TM_BEGIN();
        //paymentHome(TM_ARG warehouse_id, district_id, c_warehouse_id, c_district_id, customer_id, h_amount, now, output, undo);
        //internalPaymentRemote(TM_ARG warehouse_id, district_id, customer, h_amount, output, undo);
  
//	 Warehouse* w = findWarehouse(TM_ARG warehouse_id);

    //TM_SHARED_WRITE_F(w->w_ytd, TM_SHARED_READ_F(w->w_ytd)+h_amount);
      //TM_SHARED_WRITE_F(d->d_ytd, TM_SHARED_READ_F(d->d_ytd)+h_amount);
    TM_SHARED_WRITE_F(customer->c_balance, TM_SHARED_READ_F(customer->c_balance)-h_amount);
    TM_SHARED_WRITE_F(customer->c_ytd_payment, TM_SHARED_READ_F(customer->c_ytd_payment)+h_amount);
    TM_SHARED_WRITE(customer->c_payment_cnt, TM_SHARED_READ(customer->c_payment_cnt)+1);
...
__attribute__((transaction_safe)) void TPCCTables::internalPaymentRemote(TM_ARGDECL int64_t warehouse_id, int64_t district_id, Customer* c,
     ...
     TM_SHARED_WRITE_F(c->c_balance, TM_SHARED_READ_F(c->c_balance)-h_amount);
     TM_SHARED_WRITE_F(c->c_ytd_payment, TM_SHARED_READ_F(c->c_ytd_payment)+h_amount);
     TM_SHARED_WRITE(c->c_payment_cnt, TM_SHARED_READ(c->c_payment_cnt)+1);

      //output->c_credit_lim = TM_SHARED_READ_F(c->c_credit_lim);
      //output->c_discount = c->c_discount;
     // output->c_balance = TM_SHARED_READ_F(c->c_balance);
...
/*    __transaction_atomic { */
    ...
    for (int64_t d_id = 1; d_id <= District::NUM_PER_WAREHOUSE; ++d_id) {
        ...
        TM_SHARED_WRITE_F(c->c_balance, TM_SHARED_READ_F(c->c_balance)+total);
        TM_SHARED_WRITE(c->c_delivery_cnt, TM_SHARED_READ(c->c_delivery_cnt)+1);
    }
...
void TPCCTables::insertWarehouse(TM_ARGDECL const Warehouse& w) {
 ...
 Warehouse* TPCCTables::findWarehouse_(int64_t id) {
...
District* TPCCTables::findDistrict_(int64_t w_id, int64_t d_id) {
...
Customer* TPCCTables::findCustomer_(int64_t w_id, int64_t d_id, int64_t c_id) {
