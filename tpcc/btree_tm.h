...
public:
        ...
        __attribute__((transaction_safe)) void insert_tm(KEY key, VALUE value) {
                ...
                bool was_split;
		            long int d = TM_SHARED_READ(depth);
		            void* node = (void*)TM_SHARED_READ_P(root);
                if( d == 0 ) {
...
__attribute__((transaction_safe))  bool find_tm(const KEY& key, VALUE* value= 0) const {
  ...
  register unsigned d = TM_SHARED_READ(depth);
  register unsigned index;
  long int num_keys;
  while( d-- != 0 ) {
    inner= reinterpret_cast<const InnerNode*>(node);
    num_keys = TM_SHARED_READ(inner->num_keys);
...
__attribute__((transaction_safe)) bool findLastLessThan_tm(const KEY& key, VALUE* value = 0, KEY* out_key = 0) const {
    ...
    unsigned int d = TM_SHARED_READ(depth);
    long int num_keys;
    int64_t temp_key;
    while( d-- != 0 ) {
        ...
        temp_key = TM_SHARED_READ(inner->keys[pos-1]);
    ...
    if (pos <= num_keys) {
        pos -= 1;
	      temp_key = TM_SHARED_READ(leaf->keys[pos]);
        ...
        if (pos < num_keys) {
		    ...
		    TM_SHARED_WRITE_P(*value,temp_value);
                ...
                if (out_key != NULL) {
		    temp_key = TM_SHARED_READ(leaf->keys[pos]);
		    TM_SHARED_WRITE(*out_key,temp_key);
                }
                return true;
            } else {
...
private:
	  ...
	  //                unsigned char _pad[LEAF_NODE_PADDING];
        ...
        static unsigned leaf_position_for_tm(const KEY& key, const KEY* keys,
                ...
                unsigned k= 0;
		int64_t temp_key = TM_SHARED_READ(keys[k]);
        ...
        static inline unsigned inner_position_for_tm(const KEY& key, const KEY* keys,
                ...
                // Simple linear search. Faster for small values of N or M
                unsigned k= 0;
                long int temp_key = TM_SHARED_READ(keys[k]);
        ...
        __attribute__((transaction_safe)) bool leaf_insert_tm(LeafNode* node, KEY& key,
                ...
                long int num_keys = TM_SHARED_READ(node->num_keys);
                ...
                if( num_keys == M ) {
                        ...
                        for(unsigned j=0; j < new_sibling->num_keys; ++j) {
                                new_sibling->keys[j] = TM_SHARED_READ(node->keys[treshold+j]);
                                new_sibling->values[j] = (VALUE)TM_SHARED_READ_P(node->values[treshold+j]);
                        }
        ...
        __attribute__((transaction_safe))static void leaf_insert_nonfull_tm(LeafNode* node, KEY& key, VALUE& value,
                        unsigned index) {
		            long int temp_key = TM_SHARED_READ(node->keys[index]);
                if( temp_key == key ) {
                        // We are inserting a duplicate value.
                        // Simply overwrite the old one
			                  TM_SHARED_WRITE_P(node->values[index],value);
                } else {
                        // The key we are inserting is unique
                        unsigned i= TM_SHARED_READ(node->num_keys);
                        for(; i > index; --i) {
				                      TM_SHARED_WRITE(node->keys[i],node->keys[i-1]);
				                      TM_SHARED_WRITE_P(node->values[i],node->values[i-1]);
                        }
			i= TM_SHARED_READ(node->num_keys);
                  			TM_SHARED_WRITE(node->num_keys,i+1);
		...
		long int num_keys = TM_SHARED_READ(node->num_keys);
                if( num_keys == N ) {
                        ...
                        for(unsigned i=0; i < new_sibling->num_keys; ++i) {
                                new_sibling->keys[i]= TM_SHARED_READ(node->keys[treshold+i]);
                        ...
                        was_split= true;
                        result->key= TM_SHARED_READ(node->keys[treshold-1]);
        ...
        __attribute__((transaction_safe)) void inner_insert_nonfull_tm(InnerNode* node, unsigned current_depth, KEY& key,
                ...
                if( was_split ) {
			                  num_keys = TM_SHARED_READ(node->num_keys);
                        if( index == num_keys ) {
                                ...
                                TM_SHARED_WRITE(node->keys[index],result.key);
                        				...
                        				TM_SHARED_WRITE(node->num_keys,num_keys+1);
                        } else {
                                // Insertion not at the rightmost key
                                TM_SHARED_WRITE_P(node->children[num_keys+1],node->children[num_keys]);
                                for(unsigned i=num_keys; i!=index; --i) {
					void* temp_child = TM_SHARED_READ_P(node->children[i-1]);
                        		TM_SHARED_WRITE_P(node->children[i],temp_child);
					long int temp_key = TM_SHARED_READ(node->keys[i-1]);
                        		TM_SHARED_WRITE(node->keys[i],temp_key);
                                }
                        	TM_SHARED_WRITE_P(node->children[index],result.left);
                        	TM_SHARED_WRITE_P(node->children[index+1],result.right);
                        	TM_SHARED_WRITE(node->keys[index],result.key);
