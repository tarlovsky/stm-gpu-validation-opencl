...
static block_t* allocBlock (size_t capacity) {
    ...
    blockPtr = (block_t*)TM_MALLOC(sizeof(block_t));
    if (blockPtr == NULL) {
        return NULL;
    }
    ...
    blockPtr->contents = (char*)TM_MALLOC(capacity / sizeof(char) + 1);
...
static pool_t* allocPool (size_t initBlockCapacity, long blockGrowthFactor) {
    ...
    poolPtr = (pool_t*)TM_MALLOC(sizeof(pool_t));
    if (poolPtr == NULL) {
        return NULL;
    }
...
bool_t memory_init (long numThread, size_t initBlockCapacity, long blockGrowthFactor)  {
    ...
    global_memoryPtr = (memory_t*)TM_MALLOC(sizeof(memory_t));
    if (global_memoryPtr == NULL) {
        return FALSE;
    }

    global_memoryPtr->pools = (pool_t**)TM_MALLOC(numThread * sizeof(pool_t*));
