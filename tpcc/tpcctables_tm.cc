...
/*    __transaction_atomic { */
    ...
    int64_t o_id = TM_SHARED_READ(d->d_next_o_id);
    ...
    line = (OrderLine*)TM_SHARED_READ_P(line);
            if (line == NULL) {
...
__attribute__((transaction_safe)) void TPCCTables::internalOrderStatus(Customer* customer, OrderStatusOutput* output) {
    ...
    output->o_carrier_id = TM_SHARED_READ(order->o_carrier_id);
...
__attribute__((transaction_safe)) bool TPCCTables::newOrderHome(int64_t warehouse_id, int64_t district_id, int64_t customer_id,
    ...
    output->o_id = TM_SHARED_READ(d->d_next_o_id);
    ...
    TM_SHARED_WRITE(d->d_next_o_id, d->d_next_o_id + 1);
...
__attribute__((transaction_safe)) bool TPCCTables::newOrderRemote(int64_t home_warehouse, int64_t remote_warehouse,
	...
	int64_t stock_quantity;
	    ...
	    TM_SHARED_WRITE(stock->s_quantity,stock_quantity-item.ol_quantity);
        } else {
	    TM_SHARED_WRITE(stock->s_quantity,stock_quantity-item.ol_quantity+91);
        }
	TM_SHARED_WRITE(stock->s_ytd,stock->s_ytd+item.ol_quantity);
	TM_SHARED_WRITE(stock->s_order_cnt,stock->s_order_cnt+1);
...
__attribute__((transaction_safe)) void TPCCTables::paymentHome(int64_t warehouse_id, int64_t district_id, int64_t c_warehouse_id,
        ...
        PaymentOutput* output, TPCCUndo** undo) {
    Warehouse* w = findWarehouse(warehouse_id);
    TM_SHARED_WRITE(w->w_ytd,w->w_ytd+h_amount);

    District* d = findDistrict(warehouse_id, district_id);
    TM_SHARED_WRITE(d->d_ytd,d->d_ytd+h_amount);
...
__attribute__((transaction_safe)) void TPCCTables::internalPaymentRemote(int64_t warehouse_id, int64_t district_id, Customer* c,
    ...
    TM_SHARED_WRITE(c->c_balance,c->c_balance-h_amount);
    TM_SHARED_WRITE(c->c_ytd_payment,c->c_ytd_payment+h_amount);
    TM_SHARED_WRITE(c->c_payment_cnt,c->c_payment_cnt+1);
...
/*    __transaction_atomic { */
    ...
    for (int64_t d_id = 1; d_id <= District::NUM_PER_WAREHOUSE; ++d_id) {
        ...
        TM_SHARED_WRITE(o->o_carrier_id,carrier_id);
        ...
        TM_SHARED_WRITE(c->c_balance,c->c_balance+total);
        TM_SHARED_WRITE(c->c_delivery_cnt,c->c_delivery_cnt+1);
    }
	TM_END();
/*    } */
}
