...
unsigned int large_tx_left [CACHELINE_SIZE * sizeof(unsigned int)*THREADS_MAX];
unsigned int kick_array    [CACHELINE_SIZE * sizeof(unsigned int)*THREADS_MAX];
unsigned int intervene_pos;


void do_small(){

    /* small transaction should interfere occasionally
     * for example, intervene_pos = (array_size * intervene_%) / 100
     * */
    ...
    TM_BEGIN();/*calls stm prepare and resets tx descriptors. does not realloc*/
        /*do a small number of updates (default=4)*/
        for(int i = 1; i < nb_threads;i++){
            int kick_pos = kick_array[i*CACHELINE_SIZE];/*large tx tells us which position to kick when it reaches "intervene_pos" elements.*/
            /*if kick requested*/
            if(kick_pos > -1){
                TM_SHARED_WRITE_P(array[kick_pos], TM_SHARED_READ(array[kick_pos]));
                printf("KICKING for THREAD %d asks for kick at position %d\n", i, kick_pos);
                kick_array[i*CACHELINE_SIZE]=-1;
            }
        }
    TM_END();
...
void do_large(){
    ...
    TM_BEGIN();/*calls stm prepare and resets tx descriptors. does not realloc*/
        ...
        while (ops < chunk_size) {
            ...
            if( !kick_requested && ops >= intervene_pos ){
                /*ask small tx to kick this one; set intervene flag*/

                /*tell which element to write to with 100% accuracy*/
                kick_array[id*CACHELINE_SIZE] = i;
...
void work(void *data){
    ...
    TM_THREAD_ENTER();/*initializes tx descriptors, allocates memory, and instant kernel once*/
        if(id == 0){
            //thread 0 does kicks. KICKS: write to large tx rset occasionally.
            //write's to all tx read-set occasionally with probability p.

            int count;
            do {
                count=0;
                do_small();
                /*keep doing small tx while at least one large tx left*/
                for(int i = 1; i < nb_threads;i++){
                    count += large_tx_left[i * CACHELINE_SIZE];
                    //printf("short tx working %d\n",count);
                }
            }while(count > 0);

        }else{
            //everyone do large tx except thread 0
            /*needed in do_small to keep track of when to stop*/

            /*make large transaction*/
            for(int t = 0 ; t < LARGE_TX_N ; t++){
                do_large();
                large_tx_left[id*CACHELINE_SIZE]--;
                printf("%d: left large tx %d \n", id, large_tx_left[id*CACHELINE_SIZE]);
            }
        }
    TM_THREAD_EXIT();
    return NULL;
...
MAIN(argc, argv) {
  ...
  for(int i = 0; i < THREADS_MAX;i++){
      large_tx_left[i * CACHELINE_SIZE] = LARGE_TX_N;
      kick_array[i * CACHELINE_SIZE] = -1;
      //printf("LARGE TX LEFT IN %d: %d\n", i, large_tx_left[i * CACHELINE_SIZE]);
  }
