...
do
    ...
    ############### RANDOM ###############
    untouched="$RESULTS_DIR/TinySTM-wbetl/1/array-r99-w1-$mode-walk/1-$mode-cpu-validation"
    echo >> $FILE
    echo "set title \"array-r99-w1 ($mode ARRAY WALK)\" font \",12\"" >> $FILE
    echo "plot \\"  >> $FILE
    #memory access modes
    echo " '<paste -d \"\" $RESULTS_DIR/TinySTM-igpu-persistent-coalesced-wbetl/1/array-r99-w1-$mode-walk/1-coalesced-mem $untouched' u 0:(\$19/\$2):xtic(sprintf(\"%'d (%.2fMB)\",\$1, (((\$1*16))/1000000))) t \"Full iGPU Coalesced mem (K=RSET/5376)\" lc rgb col_48,\\" >> $FILE
    echo " '<paste -d \"\" $RESULTS_DIR/TinySTM-igpu-persistent-strided-wbetl/1/array-r99-w1-$mode-walk/1-strided-mem $untouched' u 0:(\$19/\$2):xtic(sprintf(\"%'d (%.2fMB)\",\$1, (((\$1*16))/1000000))) t \"Full iGPU Strided mem. (K=RSET/5376)\" lc rgb \"#3cde33\",\\" >> $FILE
    #untouched TINYSTM
    #echo " '$untouched' u 0:(\$2):xtic(sprintf(\"%'d (%.2fMB)\",\$1, (((\$1*16))/1000000))) t \"TiynSTM untouched\" lc rgb col_gold,\\"  >> $FILE
    #full gpu validation but in blocks of 5376
    echo " '<paste -d \"\" $RESULTS_DIR/TinySTM-igpu-persistent-blocks-wbetl/1/array-r99-w1-$mode-walk/1-$mode-igpu $untouched' u 0:(\$19/\$2):xtic(sprintf(\"%'d (%.2fMB)\",\$1, (((\$1*16))/1000000))) t \"Full iGPU, blocks of 5736 (=NO FOR LOOP to get idx), sync on block\" dt new lc rgb \"black\",\\" >> $FILE
    #co-op
    echo " '<paste -d \"\" $RESULTS_DIR/TinySTM-igpu-cpu-persistent-dynamic-split-wbetl-block-level-sync-k-1/1/array-r99-w1-$mode-walk/1-$mode-cpu-validation $untouched' u 0:(\$31/\$2):xtic(sprintf(\"%'d (%.2fMB)\",\$1, (((\$1*16))/1000000))) t \"Dynamic split K=1; blocks of 5736 on iGPU, sync on block (previous-best)\" lc rgb \"#b01313\",\\" >> $FILE
    echo " '<paste -d \"\" $RESULTS_DIR/TinySTM-igpu-cpu-persistent-dynamic-split-wbetl-block-level-sync-strided-k-4/1/array-r99-w1-$mode-walk/1-$mode-cpu-validation $untouched' u 0:(\$31/\$2):xtic(sprintf(\"%'d (%.2fMB)\",\$1, (((\$1*16))/1000000))) t \"Dynamic split K=4; blocks of 5736*4 on iGPU w/ strided mem., sync on block (new-best)\" dt new lc rgb \"#b01313\",\\" >> $FILE
    #work-item check for !valid
    echo " '<paste -d\"\" $RESULTS_DIR/TinySTM-igpu-cpu-persistent-dynamic-split-wbetl-work-item-level-sync-strided-k-4/1/array-r99-w1-$mode-walk/1-$mode-cpu-validation $untouched' u 0:(\$31/\$2):xtic(sprintf(\"%'d (%.2fMB)\",\$1, (((\$1*16))/1000000))) t \"Dynamic split K=4; gpu validates in blocks of 5736*4 w/ strided mem., sync on work-item\" dt new lc rgb \"#ea99ff\"" >> $FILE
    echo >> $FILE
