...
typedef struct {
   ...
   * Allocate a huge chunk of memory and call it clsvmvalidation.c::rset_pool
   * calls to stm_internal.h::stm_allocate_rs_entries(stm_tx_t *tx) return rset_pool[current_rset_slot]
   * */
...
extern double bench_time;
extern unsigned int time_idx;
