...
int initializeCL(volatile stm_word_t **locks_pointer){
    ...
    for (int j = 0; j < deviceCount; j++) {
        ...
        if (!strcmp(value, "gfx902+xnack")) {
            //printf("Found and using AMD gfx902+xnack (2400g Vega 11)\n");
            ...
            */
            ...
            NumberOfExecUnits = maxComputeUnits;//11 on vega
            //printf("NumberOfExecUnits: %d\n", NumberOfExecUnits);
            /* 11 Compute Units AMD, Hardware threads per EU: */
            //printf("Total hardware threads: %d\n", NumberOfExecUnits * HW_THREADS_PER_EU);
    ...
    status = initializeDeviceCode(CL_KERNEL_INSTANT, CL_KERNEL_INSTANT_NAME, BIN_KERNEL_INSTANT);
    ...
    testStatus(err, "cant get preferred workgroup size");
...
int initializeDeviceCode(const char* kernel_src_path, const char* kernel_name, const char* bin_location){
    cl_program program;//temporary to create binary.
    cl_kernel kernel;
    ...
    int kernel_ready = 0;//for first bin compile only
    int length;
    size_t binary_size;
    
    // if binary kernel file does not exist compile program
    if( access( bin_location, F_OK ) != -1 ){
        /*load binary from file*/
        filename = bin_location;
        //printf("binary file path %s\n", filename);
        bin_to_str(filename, &length); // bin_to_str populates g_clProgramString with binary from .bin
        binary_size = length;
    } else {
        /*binary InstantKernel.bin does not exist. Create binary from InstantKernel.cl file*/
        ...
        program = clCreateProgramWithSource(g_clContext, 1, (const char **) &g_clProgramString, &sourceSize, &status);
        clBuildProgram(program, 1, &g_clDevices[0], CL_KERNEL_PROGRAM_OPTIONS, NULL, NULL);
        kernel=clCreateKernel(program, kernel_name, NULL);
        clGetProgramInfo(program, CL_PROGRAM_BINARY_SIZES, sizeof(size_t), &binary_size, NULL);
        g_clProgramString = malloc(binary_size);//reuse g_clProgramString to write binary program into it.
        status = clGetProgramInfo(program, CL_PROGRAM_BINARIES, binary_size, &g_clProgramString, NULL);

        f = fopen(bin_location, "w");
    ...
    clBuildProgram(g_clProgram, 1, g_clDevices, CL_KERNEL_PROGRAM_OPTIONS, NULL, NULL);
...
int launchInstantKernel(void){
    ...
    */
    ...
    while (pCommBuffer[SPIN] < NumberOfHwThreads){//176 HW THREADS IMPIRICALLTY IN VEGA 11 2400G
...
char* bin_to_str(const char *fileName, long *length_out){
    ...
    /*read into global program string char array*/
