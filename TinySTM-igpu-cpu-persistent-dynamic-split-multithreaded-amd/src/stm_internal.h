...
# define FILTER_B2ITS(a)                 (1 << (FILTER_HASH(a) & 0x1F))
...
typedef struct stm_tx {                 /* Transaction descriptor */
  ...
  /* tarlovskyy */
  volatile unsigned int rset_slot;
  ...
  unsigned long long gpu_validated;     /* at least 64 bits */
...
typedef struct {
  /* volatile stm_word_t locks[LOCK_ARRAY_SIZE] ALIGNED; //original */
  /* I keep it aligned in clSVMAlloc */
...
extern global_t _tinystm;//no memory reserved yet. only declaration
...
/*tarlovsky custom get_time() to meause validation calls*/
static double query_performance()
{
  struct timeval t;
  struct timezone tzp;
  gettimeofday(&t, &tzp);
...
stm_allocate_rs_entries(stm_tx_t *tx, int extend)
{
  ...
  if (extend) {
    //TODO realloc in special svm space.
    //asserts
...
int_stm_init_thread(void)
{
  ...
  /* to reduce lock grab attempts check flag */
    ...
    if( _tinystm.kernel_init==0
      ...
      //TIMER_T stop;
      //TIMER_READ(start);
      pthread_mutex_lock(&_tinystm.kernel_init_mutex);
      if(_tinystm.kernel_init==0){
          /* if while we were grabbing lock someone set it to one check again */
          launchInstantKernel();
          _tinystm.kernel_init = INITIAL_RS_SVM_BUFFERS_OCL;//only release kernel when all threads decrement.
      }
  ...
  tx->cpu_validated = 0;
  tx->gpu_validated = 0;
  tx->waste_double_validated = 0;
  tx->gpu_employed_times = 0;
  tx->stat_val_succ = 0;
  tx->stat_val_fail = 0;
  tx->val_time = 0.0;
  tx->cpu_val_time = 0.0;
...
int_stm_exit_thread(stm_tx_t *tx)
{
    ...
    //_tinystm.kernel_init--;

    //if(_tinystm.kernel_init==0){
        /*  */
        //printf("FINISHING KERNEL %d\n", _tinystm.kernel_init);
...
int_stm_get_stats(stm_tx_t *tx, const char *name, void *val)
{
  ...
  if (strcmp("cpu_val_time", name) == 0) {
    *(double *)val = tx->cpu_val_time;
    return 1;
  }
  if (strcmp("gpu_val_time", name) == 0) {
    *(double *)val = tx->gpu_val_time;
    return 1;
    return 1;
  }
  ...
  if (strcmp("cpu_validated", name) == 0) {
    *(unsigned long long *)val = tx->cpu_validated;
    return 1;
  }
  if (strcmp("gpu_validated", name) == 0) {
    *(unsigned long long *)val = tx->gpu_validated;
    return 1;
  }
  if (strcmp("waste_double_validated", name) == 0) {
    *(unsigned long long *)val = tx->waste_double_validated;
    return 1;
  }
  if (strcmp("gpu_employed_times", name) == 0) {
    *(unsigned long long *)val = tx->gpu_employed_times;
    return 1;
  }
