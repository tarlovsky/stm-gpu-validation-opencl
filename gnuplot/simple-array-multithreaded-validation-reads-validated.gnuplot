...
set ylabel "READS VALIDATED / VALIDATION CALL / SECOND"
set arrow from 5.8, graph 0 to 5.8, graph 1 nohead lc rgb "#efefef"
set label "$L1: 128KB" at 5.9, 10000000 
set arrow from 8.8, graph 0 to 8.8, graph 1 nohead lc rgb "#bebebe"
set label "$L2: 1.024MB" at 8.9, 10000000+10000000*0.50 
set arrow from 11.8, graph 0 to 11.8, graph 1 nohead lc rgb "#afafaf"
set label "$L3: 8MB" at 11.9, 10000000+10000000*2 
set title "Only CPU, threaded validation, sequential walk" font ",12"
set title "1 STM threads" font ",12"
plot \
 'results-validation-array/TinySTM-igpu-cpu-persistent-wbetl/1/array-r99-w1-random-walk/1-random-cpu-67-gpu-33' u ($0):((($14+$16)>0)?((($12/(ceil($14+$16)))/$2)/1):(NaN)):xtic(sprintf("%'d (%.2fMB)",$1, ($1*8)/1000000)) t "1 STM THREADS GLOBAL MINUMUM 1-random-cpu-67-gpu-33" lw 2 lc rgb "#b01313",\
...
u ($0):((($22+$24)>0)?((($12/(ceil($22+$24)))/$2)/1):(NaN)):xtic(sprintf("%'d (%.2fMB)",$1, ($1*8)/1000000)) t "1 STM CPU+GPU DYNAMIC CO-OP - BLOCKS" lw 2 lc rgb "#69a2ff" ,\
 'results-validation-array/TinySTM-igpu-persistent-blocks-wbetl/1/array-r99-w1-random-walk/1-random-igpu' u ($0):((($10+$12)>0)?( (($8/(ceil($10+$12)))/1) /  ($2/1) ):(NaN)):xtic(sprintf("%'d (%.2fMB)",$1, ($1*8)/1000000)) t "Intel iGPU HD530 PERSISTENT KERNEL - BLOCKS" dt new lw 2 lc rgb "#69a2ff" ,\
