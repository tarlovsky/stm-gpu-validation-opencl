...
unset rmargin
unset lmargin
set multiplot layout 2,2 title "Random array HOW MANY READS VALIDATED PER VALIDATION CALL PER THREAD, DIVIDED OVER TOTAL VALIDATION TIME ON A THREAD; (ARRAY WALK HAS ONLY ONE TRANSACTION)" font ",12"
set tics scale 0
set ytics
...
col_gold="#8f8800"
set key font ",8"
set key left Left left Left inside top
set ylabel "READS VALIDATED / THREAD / CALL / TOTAL VALIDATION TIME"
...
plot \
 'results-validation-array/TinySTM-igpu-cpu-persistent-wbetl/1a/array-r99-w1-random-walk/1-random-cpu-67-gpu-33' u ($0):((($10+$12)>0)?( (($8/(ceil($10+$12)))/1) /  ($2/1) ):(NaN)):xtic(sprintf("%d/ %.2fMB",$1, ($1*8)/1000000)) t "GLOBAL MINUMUM 1-random-cpu-67-gpu-33" dt new lc rgb "#b01313",\
