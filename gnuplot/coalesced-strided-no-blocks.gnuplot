...
plot \
 'results-validation-array/TinySTM-igpu-persistent-coalesced-wbetl/1/array-r99-w1-sequential-walk/1-coalesced-mem' u 0:2:3:xtic(sprintf("%'d (%.2fMB)",$1, ((($1*16))/1000000))) with yerrorlines t "Full iGPU Coalesced mem (K=RSET/5376)" lc rgb col_48,\
 'results-validation-array/TinySTM-igpu-persistent-strided-wbetl/1/array-r99-w1-sequential-walk/1-strided-mem' u 0:2:3:xtic(sprintf("%'d (%.2fMB)",$1, ((($1*16))/1000000))) with yerrorlines t "Full iGPU Strided mem. (K=RSET/5376)" lc rgb "#3cde33",\
 ...
 'results-validation-array/TinySTM-igpu-persistent-blocks-wbetl/1/array-r99-w1-sequential-walk/1-sequential-igpu' u 0:2:3:xtic(sprintf("%'d (%.2fMB)",$1, ((($1*16))/1000000))) with yerrorlines t "Full iGPU, blocks of 5736 (=NO FOR LOOP to get idx), sync on block" dt new lc rgb "black",\
 'results-validation-array/TinySTM-igpu-cpu-persistent-dynamic-split-wbetl-block-level-sync-k-1/1/array-r99-w1-sequential-walk/1-sequential-cpu-validation' u 0:2:3:xtic(sprintf("%'d (%.2fMB)",$1, ((($1*16))/1000000))) with yerrorlines t "Dynamic split K=1; blocks of 5736 on iGPU, sync on block (previous-best)" lc rgb "#b01313",\
