set terminal wxt size 1300,1200
unset bmargin
unset tmargin
unset rmargin
unset lmargin
...
set label 2"$L1: 128KB" at 5.9, 0.54 font "Computer Modern, 14"
set arrow 4 from 8.8, graph 0 to 8.8, graph 1 nohead lc rgb "#bebebe"
set label 4"$L2: 1.024MB" at 8.9, 0.62 font "Computer Modern, 15"
set arrow 5 from 11.8, graph 0 to 11.8, graph 1 nohead lc rgb "#afafaf"
set label 5"$L3: 8MB" at 11.9, 0.71 font "Computer Modern, 15"
...
plot \
 '< join ../results-validation-array/TinySTM-wbetl/1/array-r99-w1-sequential-walk/1-sequential-cpu-validation ../results-validation-array/TinySTM-threads-wbetl/1/array-r99-w1-sequential-walk/1-sequential-cpu-validation-8-workers' u ($0):(($24/($18*1)) / ($8/($2*1))):( sprintf( '%.2fx',(($24/($18*1)) / ($8/($2*1))) ) ):xtic(sprintf("%'d ",$1, ($1*8)/1000000)) t "8 validator threads" lw 3 with linespoints,\
...
plot \
 '< join ../results-validation-array/TinySTM-wbetl/2/array-r99-w1-sequential-walk/2-sequential-cpu-validation ../results-validation-array/TinySTM-threads-wbetl/2/array-r99-w1-sequential-walk/2-sequential-cpu-validation-8-workers' u ($0):(($24/($18*2)) / ($8/($2*2))):( sprintf( '%.2fx',(($24/($18*2)) / ($8/($2*2))) ) ):xtic(sprintf("%'d ",$1, ($1*8)/1000000)) t "8 validator threads" lw 3 with linespoints,\
...
plot \
 '< join ../results-validation-array/TinySTM-wbetl/4/array-r99-w1-sequential-walk/4-sequential-cpu-validation ../results-validation-array/TinySTM-threads-wbetl/4/array-r99-w1-sequential-walk/4-sequential-cpu-validation-8-workers' u ($0):(($24/($18*4)) / ($8/($2*4))):( sprintf( '%.2fx',(($24/($18*4)) / ($8/($2*4))) ) ):xtic(sprintf("%'d ",$1, ($1*8)/1000000)) t "8 validator threads" lw 3 with linespoints,\
...
plot \
 '< join ../results-validation-array/TinySTM-wbetl/8/array-r99-w1-sequential-walk/8-sequential-cpu-validation ../results-validation-array/TinySTM-threads-wbetl/8/array-r99-w1-sequential-walk/8-sequential-cpu-validation-8-workers' u ($0):(($24/($18*8)) / ($8/($2*8))):( sprintf( '%.2fx',(($24/($18*8)) / ($8/($2*8))) ) ):xtic(sprintf("%'d ",$1, ($1*8)/1000000)) t "8 validator threads" lw 3 with linespoints,\
