...
plot \
 ...
 'results-validation-array/TinySTM-igpu-persistent-coalesced-wbetl/1a-array-r99-w1-random-GPU-24WKGPS-224WKGPSIZE-ACQ-REL' u 3:xtic(sprintf("%d/ %.2fMB",$1, ($1*8)/1000000)) t "Normal execution (GPU)" dt new lc rgb col_24 pt 1,\
 'results-validation-array/TinySTM-igpu-persistent-coalesced-wbetl/1a-array-r99-w1-random-GPU-24WKGPS-224WKGPSIZE-ACQ-REL' u 4:xtic(sprintf("%d/ %.2fMB",$1, ($1*8)/1000000)) t "No branching logic (GPU)" dt new1 lc rgb col_24 pt 1,\
 'results-validation-array/TinySTM-igpu-persistent-coalesced-wbetl/1a-array-r99-w1-random-GPU-24WKGPS-224WKGPSIZE-ACQ-REL' u 5:xtic(sprintf("%d/ %.2fMB",$1, ($1*8)/1000000)) t "work-items don't load lock, load read-entry.(GPU)" lw 1 lc rgb col_24 pt 1,\
 'results-validation-array/TinySTM-igpu-persistent-coalesced-wbetl/1a-array-r99-w1-random-GPU-24WKGPS-224WKGPSIZE-ACQ-REL' u 6:xtic(sprintf("%d/ %.2fMB",$1, ($1*8)/1000000)) t "Work-items don't load read-entry. Only calculate for loop start-end." dt new lc rgb col_24 pt 8,\
 'results-validation-array/TinySTM-igpu-persistent-coalesced-wbetl/1a-array-r99-w1-random-GPU-24WKGPS-224WKGPSIZE-ACQ-REL' u 7:xtic(sprintf("%d/ %.2fMB",$1, ($1*8)/1000000)) t "No validation inside kernel. Basic kernel polling (GPU) in every WI" dt new lc rgb "black" pt 8,\
