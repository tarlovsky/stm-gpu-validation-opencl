...
set bmargin 9
set lmargin 7
set multiplot layout 2,2 title "Contention on READS VALIDATED/S, normalized to TinySTM-untouched-Intel; INTEL-COOP - CAS COMPETE FOR IGPU" font ",20"
col_24="#c724d6"
col_48="#44cd1"
col_gold="#8f8800"
col_red="#b01313"
...
plot \
 '< join ../results-validation-array/TinySTM-wbetl/1/array-strongly-scaled-r99-w1-d1-random-walk/1-random ../results-validation-array/TinySTM-igpu-cpu-persistent-dynamic-split-multithreaded-wbetl/1/array-strongly-scaled-shared-gpu-r99-w1-d1-random-walk/1-random' u ($0):(($28/($18*1)) / ($8/($2*1))):( sprintf( '%.2fx',(($28/($18*1)) / ($8/($2*1))) ) ):xtic(sprintf("%'d (%.2fMB)",$1, ($1*8)/1000000)) t "No contention (disjoint arrays)" pt 2 lw 2 lc rgb "#94bdff" dt new with linespoints,\
 '< join ../results-validation-array/TinySTM-wbetl/1/array-strongly-scaled-r99-w1-d1-random-walk/1-random ../results-validation-array/TinySTM-igpu-cpu-persistent-dynamic-split-multithreaded-wbetl/1/array-strongly-scaled-shared-gpu-r99-w1-d0-random-walk/1-random' u ($0):(($28/($18*1)) / ($8/($2*1))):( sprintf( '%.2fx',(($28/($18*1)) / ($8/($2*1))) ) ):xtic(sprintf("%'d (%.2fMB)",$1, ($1*8)/1000000)) t "Medium contention (conjoint array)" pt 2 lw 2 lc rgb "#94bdff" with linespoints,\

set title "2 STM threads" offset 0, -1.15 font "Computer Modern,23"
plot \
 '< join ../results-validation-array/TinySTM-wbetl/2/array-strongly-scaled-r99-w1-d1-random-walk/2-random ../results-validation-array/TinySTM-igpu-cpu-persistent-dynamic-split-multithreaded-wbetl/2/array-strongly-scaled-shared-gpu-r99-w1-d1-random-walk/2-random' u ($0):(($28/($18*2)) / ($8/($2*2))):( sprintf( '%.2fx',(($28/($18*2)) / ($8/($2*2))) ) ):xtic(sprintf("%'d (%.2fMB)",$1, ($1*8)/1000000)) t "No contention (disjoint arrays)" pt 2 lw 2 lc rgb "#94bdff" dt new with linespoints,\
 '< join ../results-validation-array/TinySTM-wbetl/2/array-strongly-scaled-r99-w1-d1-random-walk/2-random ../results-validation-array/TinySTM-igpu-cpu-persistent-dynamic-split-multithreaded-wbetl/2/array-strongly-scaled-shared-gpu-r99-w1-d0-random-walk/2-random' u ($0):(($28/($18*2)) / ($8/($2*2))):( sprintf( '%.2fx',(($28/($18*2)) / ($8/($2*2))) ) ):xtic(sprintf("%'d (%.2fMB)",$1, ($1*8)/1000000)) t "Medium contention (conjoint array)" pt 2 lw 2 lc rgb "#94bdff" with linespoints,\

set title "4 STM threads" offset 0, -1.15 font "Computer Modern,23"
plot \
 '< join ../results-validation-array/TinySTM-wbetl/4/array-strongly-scaled-r99-w1-d1-random-walk/4-random ../results-validation-array/TinySTM-igpu-cpu-persistent-dynamic-split-multithreaded-wbetl/4/array-strongly-scaled-shared-gpu-r99-w1-d1-random-walk/4-random' u ($0):(($28/($18*4)) / ($8/($2*4))):( sprintf( '%.2fx',(($28/($18*4)) / ($8/($2*4))) ) ):xtic(sprintf("%'d (%.2fMB)",$1, ($1*8)/1000000)) t "No contention (disjoint arrays)" pt 2 lw 2 lc rgb "#94bdff" dt new with linespoints,\
 '< join ../results-validation-array/TinySTM-wbetl/4/array-strongly-scaled-r99-w1-d1-random-walk/4-random ../results-validation-array/TinySTM-igpu-cpu-persistent-dynamic-split-multithreaded-wbetl/4/array-strongly-scaled-shared-gpu-r99-w1-d0-random-walk/4-random' u ($0):(($28/($18*4)) / ($8/($2*4))):( sprintf( '%.2fx',(($28/($18*4)) / ($8/($2*4))) ) ):xtic(sprintf("%'d (%.2fMB)",$1, ($1*8)/1000000)) t "Medium contention (conjoint array)" pt 2 lw 2 lc rgb "#94bdff" with linespoints,\
...
plot \
 '< join ../results-validation-array/TinySTM-wbetl/8/array-strongly-scaled-r99-w1-d1-random-walk/8-random ../results-validation-array/TinySTM-igpu-cpu-persistent-dynamic-split-multithreaded-wbetl/8/array-strongly-scaled-shared-gpu-r99-w1-d1-random-walk/8-random' u ($0):(($28/($18*8)) / ($8/($2*8))):( sprintf( '%.2fx',(($28/($18*8)) / ($8/($2*8))) ) ):xtic(sprintf("%'d (%.2fMB)",$1, ($1*8)/1000000)) t "No contention (disjoint arrays)" pt 2 lw 2 lc rgb "#94bdff" dt new with linespoints,\
 '< join ../results-validation-array/TinySTM-wbetl/8/array-strongly-scaled-r99-w1-d1-random-walk/8-random ../results-validation-array/TinySTM-igpu-cpu-persistent-dynamic-split-multithreaded-wbetl/8/array-strongly-scaled-shared-gpu-r99-w1-d0-random-walk/8-random' u ($0):(($28/($18*8)) / ($8/($2*8))):( sprintf( '%.2fx',(($28/($18*8)) / ($8/($2*8))) ) ):xtic(sprintf("%'d (%.2fMB)",$1, ($1*8)/1000000)) t "Medium contention (conjoint array)" pt 2 lw 2 lc rgb "#94bdff" with linespoints,\
