...
unset rmargin
unset lmargin
set multiplot layout 2,2 title "CONJOINT array walk: TRANSACTIONAL THROUGHPUT NORMALIZED TO TINYSTM-UNTOUCHED; MULTITHREADED STM - CAS SHARED IGPU" font ",13"
col_24="#c724d6"
col_48="#44cd1"
col_gold="#8f8800"
col_red="#b01313"
...
plot \
 ...
 '< join results-validation-array/TinySTM-wbetl/1/array-strongly-scaled-r99-w1-d0-random-walk/1-random results-validation-array/TinySTM-igpu-cpu-persistent-dynamic-split-multithreaded-wbetl-lsa/1/array-strongly-scaled-shared-gpu-r99-w1-d0-random-walk/1-random' u ($0):(($24/($46*1)) / ($4/($16*1))):( sprintf( '%.2fx',(($24/($46*1)) / ($8/($2*1))) ) ):xtic(sprintf("%'d (%.2fMB)",$1, ($1*8)/1000000)) t "CPU-GPU co-op; iGPU shared GPU (iGPU CAS); Blocks of 5736 on iGPU; sync on block; LSA" lc rgb col_red dt new with linespoints,\
...
plot \
 ...
 '< join results-validation-array/TinySTM-wbetl/2/array-strongly-scaled-r99-w1-d0-random-walk/2-random results-validation-array/TinySTM-igpu-cpu-persistent-dynamic-split-multithreaded-wbetl-lsa/2/array-strongly-scaled-shared-gpu-r99-w1-d0-random-walk/2-random' u ($0):(($24/($46*2)) / ($4/($16*2))):( sprintf( '%.2fx',(($24/($46*2)) / ($8/($2*2))) ) ):xtic(sprintf("%'d (%.2fMB)",$1, ($1*8)/1000000)) t "CPU-GPU co-op; iGPU shared GPU (iGPU CAS); Blocks of 5736 on iGPU; sync on block; LSA" lc rgb col_red dt new with linespoints,\
...
plot \
 ...
 '< join results-validation-array/TinySTM-wbetl/4/array-strongly-scaled-r99-w1-d0-random-walk/4-random results-validation-array/TinySTM-igpu-cpu-persistent-dynamic-split-multithreaded-wbetl-lsa/4/array-strongly-scaled-shared-gpu-r99-w1-d0-random-walk/4-random' u ($0):(($24/($46*4)) / ($4/($16*4))):( sprintf( '%.2fx',(($24/($46*4)) / ($8/($2*4))) ) ):xtic(sprintf("%'d (%.2fMB)",$1, ($1*8)/1000000)) t "CPU-GPU co-op; iGPU shared GPU (iGPU CAS); Blocks of 5736 on iGPU; sync on block; LSA" lc rgb col_red dt new with linespoints,\
