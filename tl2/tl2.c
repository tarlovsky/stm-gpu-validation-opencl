...
#ifdef TL2_STATS
    /*tarlovsky*/
    printf("%lf %d %d %llu %d %d\n", t->val_time, t->Commits, t->Aborts, t->val_reads, t->val_succ, t->val_fail);
#endif
...
#  ifdef TL2_EAGER
            ...
            if (p->Owner == Self) {
                if (p->rdv > rv) {
                    #ifdef TL2_STATS
                    TIMER_READ(stop);
                    Self->val_time += TIMER_DIFF_SECONDS(start, stop);
                    Self->val_fail++;
                    #endif
...
#ifdef TL2_STATS
    /* if get time here than all validation passed and tx can validate*/
    TIMER_READ(stop);
    Self->val_time += TIMER_DIFF_SECONDS(start, stop);
    if(dx == 0){
        Self->val_succ++;
...
{
    //printf("%d ReadSetCoherentPessimistic ", Self->UniqID);
    vwLock rv = Self->rv;
    Log* const rd = &Self->rdSet;
    AVPair* const EndOfList = rd->put;
    AVPair* e;
...
TxAbort (Thread* Self)
{
    //printf("Aborting val id %d A %d\n", Self->UniqID, Self->Aborts);
...
#    else /* !TL2_NOCM */
        ...
        for (;;) {
            ...
            if (--c < 0) {
                PROF_STM_WRITE_END();
                //printf("%d STM WRITE END", Self->UniqId);
                TxAbort(Self);
                ASSERT(0);
            }
        }
    ...
    if (ALWAYS && LDNF(addr) == valu) {
        ...
        if ((rdv & LOCKBIT) == 0 && rdv <= Self->rv && LDLOCK(LockFor) == rdv) {
            if (!TrackLoad(Self, LockFor)) {
                PROF_STM_WRITE_END();
                //printf("%d Not writing new value; convert to load", Self->UniqID);
                TxAbort(Self);
            }
            PROF_STM_WRITE_END();
            return;
...
#  endif /* !TL2_OPTIM_HASHLOG */
        ...
        for (e = wr->tail; e != NULL; e = e->Prev) {
            ...
            if (e->Addr == Addr) {
                PROF_STM_READ_END();
                //printf("%d e->Valu from own writeset\n", Self->UniqID);
                //return, it's in my write set.
                return e->Valu;
            }
        }
    }
    ...
    if (rdv <= Self->rv && LDLOCK(LockFor) == rdv) {
        ...
        PROF_STM_READ_END();
        //printf("Should be entering here\n");
        return Valu;
    }
    ...
    Self->abv = rdv;
    PROF_STM_READ_END();
    //printf("%d Location locked, contention\n", Self->UniqID);
    TxAbort(Self);
    ASSERT(0);
...
TxCommit (Thread* Self)
{
    ...
    //printf("Tryint to commit are we ?\n");
    ASSERT(Self->Mode == TTXN);
