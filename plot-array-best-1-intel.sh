...
echo "set title \"VARYING WORKGROUPS;(WORKITEMS/WKGP) MAINTAINING FULL OCCUPANCY - FULL GPU VALIDATION\" font \",12\"" >> $FILE
echo  "plot \\"  >> $FILE
...
echo "set ytics 100000000" >> $FILE
#BEST INTEL PERSISTENT KERNEL COALESCED
echo "set title \"STRIDED VS COALESCED MEM ACCESS - FULL GPU VALIDATION\" font \",12\"" >> $FILE
echo  "plot \\"  >> $FILE
#COALESCED mem access VARY # OF WORKGROUPS
echo  " '$RESULTS_DIR/TinySTM-igpu-persistent-coalesced-wbetl/1/array-r99-w1-random-walk/1-random-cpu-validation-24wkgps-224wi-each-acq-rel'    u (\$0):(\$8/\$2):xtic(sprintf(\"%'d (%.2fMB)\",\$1, (((\$1*8))/1000000))) t \"Intel HD 530 coalesced 24WKGPS-224WKGPSIZE-ACQ-REL\" lw 1 pt 2 lc rgb col_48,\\"  >> $FILE
...
echo "set ytics 100000000" >> $FILE
#BEST INTEL PERSISTENT KERNEL COALESCED
echo "set title \"VARYING READS VALIDATED PER WORK-ITEM COALESCED - FULL GPU VALIDATION\" font \",12\"" >> $FILE
echo  "plot \\"  >> $FILE

#blocks
for w in ${KARRAY_INTEL[@]}; do
  if [[ $w -eq 2 ]]; then
    echo  " '$RESULTS_DIR/TinySTM-igpu-persistent-blocks-wbetl/1/array-r99-w1-random-walk/1-coalesced-mem-K-$w'    u (\$0):(\$8/\$2):xtic(sprintf(\"%'d (%.2fMB)\",\$1, (((\$1*8))/1000000))) t \"Blocks of 5376*$w\" lw 2,\\"  >> $FILE
  else
...
echo "set ytics 100000000" >> $FILE
#BEST INTEL PERSISTENT KERNEL COALESCED
echo "set title \"VARYING READS VALIDATED PER WORK-ITEM STRIDED - FULL GPU VALIDATION\" font \",12\"" >> $FILE
echo  "plot \\"  >> $FILE

for w in ${KARRAY_INTEL[@]}; do
  if [[ $w -eq 2 ]]; then
  echo  " '$RESULTS_DIR/TinySTM-igpu-persistent-blocks-wbetl/1/array-r99-w1-random-walk/1-strided-mem-K-$w'    u (\$0):(\$8/\$2):xtic(sprintf(\"%'d (%.2fMB)\",\$1, (((\$1*8))/1000000))) t \"Blocks of 5376*$w\" lw 2 ,\\"  >> $FILE
  else
