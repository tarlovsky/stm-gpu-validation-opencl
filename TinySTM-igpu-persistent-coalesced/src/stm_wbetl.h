...
static INLINE int
stm_wbetl_validate_mad(stm_tx_t *tx)
{
    ...
    /* Just printing locks to compare to clsvmvalidation.c and lock values inside kernel through debug buffer */
    /*for(int i = 0; i < LOCK_ARRAY_SIZE; i++){
      if(_tinystm.locks[i] != 0) {
          printf("i: %d _tinystm.locks: 0x%016"PRIxPTR"\n", i, _tinystm.locks[i]);
      }
    }*/
    /*r_entry_t *r = (r_entry_t*) r_entry_pool[tx->rset_slot];
    int i;
    for (i = 0; i < tx->r_set.nb_entries; i++, r++) {
        printf("i: %d tx->rset: 0x%016"PRIxPTR"\n", i, ATOMIC_LOAD(r->lock));
    }*/
    ...
    if(tx->r_set.nb_entries > TUNABLE){
        /*If you want to compute the elapsed time between two events observed on the one machine
        * without an intervening reboot, CLOCK_MONOTONIC is the best option.*/
        TIMER_READ(start);

        v = signal_gpu(tx->rset_slot, tx->r_set.nb_entries, tx->w_set.entries, tx->w_set.entries + tx->w_set.nb_entries, &tx->val_reads);
        ...
        r = (r_entry_t*) r_entry_pool[tx->rset_slot];
        //r = tx->r_set.entries;
    ...
    //todo iterate 1000 times and compare against untouched iterated 1000 times. Same cached readset validation.
ret:
    TIMER_READ(stop);
    //printf("v %d %fs\n", v, TIMER_DIFF_SECONDS(start, stop));
    tx->val_time += TIMER_DIFF_SECONDS(start, stop);
    tx->stat_val_succ += v;//faster than a branch i guess, v is either 1 or 0
    tx->stat_val_fail += !v;//
    //printf("VALID READ SET %d\n",v);
