...
int initializeDeviceData(){
    ...
    int alloc_err = posix_memalign((void**) locks, CACHELINE_SIZE, LOCK_ARRAY_SIZE * sizeof(stm_word_t));
    ...
    r_entry_pool = (r_entry_t**) malloc(initial_rs_svm_buffers_ocl_global * sizeof(r_entry_t*));
     ...
     for(int i = 0; i < initial_rs_svm_buffers_ocl_global; i++) {
        ...
        alloc_err = posix_memalign((void**) &r_entry_pool[i], CACHELINE_SIZE, RW_SET_SIZE * sizeof(r_entry_t));

        posix_memalign_test(alloc_err, "rset_pool");

    }
    
    r_entry_pool_cl_wrapper = (r_entry_wrapper_t*) malloc(initial_rs_svm_buffers_ocl_global * sizeof(r_entry_wrapper_t*));
