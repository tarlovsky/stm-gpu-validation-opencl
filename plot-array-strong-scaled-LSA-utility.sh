...
echo "set terminal wxt size 1400,550" > $FILE1
echo "set terminal wxt size 1400,550" > $FILE2
echo "set terminal wxt size 1400,550" > $FILE3
...
echo "set multiplot layout 1,2 title \"DISJOINT array walk, 100 rounds, write to neighbour once per round: TIME % SPENT IN VALIDATION\" font \",14\"" >> $FILE1
echo "set multiplot layout 1,2 title \"DISJOINT array walk: 100 rounds, write to neighbour once per round: ABORTS/s \" font \",13\"" >> $FILE2
echo "set multiplot layout 1,2 title \"DISJOINT array walk: 100 rounds, write to neighbour once per round: TRANSACTIONS/s \" font \",13\"" >> $FILE3
...
#echo "set yrange [0.5:2.0]" | tee -a $FILE2
echo "set yrange [0.5:1.5]" | tee -a $FILE3
...
echo "set ylabel \"#ABORTS\"" | tee -a $FILE2
...
for i in 4 8; do
  ...
  coop_lsa="$RESULTS_DIR/TinySTM-igpu-cpu-persistent-dynamic-split-multithreaded-wbetl/$i/array-strongly-scaled-all-large-tx-shared-gpu-r99-w1-d1-RR-kick-random-walk/$i-random"
  ...
  # "RSET" "Validation time (s)" "stddev" "Commits" "stddev" "Aborts" "stddev" "Val Reads" "stddev" "Val success" "stddev" "Val fail" "stddev" "Energy (J)" "stddev" "Total time (s)" "stddev"
  echo  " '$tiny' u (\$0):(\$2/(\$16*$i)):( sprintf( '%.2fx', (\$2/(\$16*$i)) ) ):xtic(sprintf(\"%'d (%.2fMB)\", \$1, (\$1*8)/1000000)) t \"TinySTM-wbetl\" with linespoints lc rgb col_gold,\\"  >> $CHOICE_VAL_PROP
  echo  " '$tiny_lsa' u (\$0):(\$2/(\$16*$i)):( sprintf( '%.2fx', (\$2/(\$16*$i)) ) ):xtic(sprintf(\"%'d (%.2fMB)\", \$1, (\$1*8)/1000000)) t \"TinySTM-wbetl-lsa\" with linespoints lc rgb col_gold dt new,\\"  >> $CHOICE_VAL_PROP
  # "RSET" "Validation time (s)" "stddev" "Validation time (s) CPU" "stddev" "Validation time (s) GPU" "stddev" "Commits" "stddev" "Aborts" "stddev" "Val Reads" "stddev" "CPU Val Reads" "stddev" "GPU Val Reads" "stddev" "Wasted Val Reads" "stddev" "GPU employment times" "stddev" "Val success" "stddev" "Val fail" "stddev" 26"Snapshot ext. calls" "stddev" "Energy (J)" "stddev" #30:"Total time (s)" "stddev"
  echo  " '$coop' u (\$0):(\$2/(\$30*$i)):( sprintf( '%.2fx',(\$2/(\$30*$i)) ) ):xtic(sprintf(\"%'d (%.2fMB)\",\$1, (\$1*8)/1000000)) t     \"CPU-GPU co-op; iGPU shared (CAS compete); Blocks of 5736 on iGPU; sync on block\" lc rgb col_red        with linespoints,\\"  >> $CHOICE_VAL_PROP
  echo  " '$coop_lsa' u (\$0):(\$2/(\$30*$i)):( sprintf( '%.2fx',(\$2/(\$30*$i)) ) ):xtic(sprintf(\"%'d (%.2fMB)\",\$1, (\$1*8)/1000000)) t \"LSA CPU-GPU co-op; iGPU shared (CAS compete); Blocks of 5736 on iGPU; sync on block\" lc rgb col_red dt new with linespoints,\\"  >> $CHOICE_VAL_PROP
  # "RSET" "Validation time (s)" "stddev" "Commits" "stddev" "Aborts" "stddev" "Val Reads" "stddev" "Val success" "stddev" "Val fail" "stddev" "Energy (J)" "stddev" "Total time (s)" "stddev"
  echo  " '$tiny_threads' u (\$0):(\$2/(\$16*$i)):( sprintf( '%.2fx',(\$2/(\$16*$i)) ) ):xtic(sprintf(\"%'d (%.2fMB)\",\$1, (\$1*8)/1000000)) t \"TinySTM-wbetl 4 validation worker threads\" lc rgb \"#${gray_palette[2]}\" with linespoints,\\"  >> $CHOICE_VAL_PROP
  echo  " '$tiny_threads_lsa' u (\$0):(\$2/(\$16*$i)):( sprintf( '%.2fx',(\$2/(\$16*$i)) ) ):xtic(sprintf(\"%'d (%.2fMB)\",\$1, (\$1*8)/1000000)) t \"LSA; TinySTM-wbetl 4 validation worker threads\" lc rgb \"#${gray_palette[2]}\" dt new1 with linespoints,\\"  >> $CHOICE_VAL_PROP
  ##########################


  # TX/S relative to tiny-untouched
  ...
  echo  " '< join $tiny $coop' u (\$0):((\$24/(\$46*$i)) / (\$4/(\$16*$i))):( sprintf( '%.2fx',((\$24/(\$46*$i)) / (\$4/(\$16*$i))) ) ):xtic(sprintf(\"%'d (%.2fMB)\",\$1, (\$1*8)/1000000)) t \"CPU-GPU co-op; iGPU shared (CAS compete); Blocks of 5736 on iGPU; sync on block\" lc rgb col_red with linespoints,\\"  >> $CHOICE_TXPS
  echo  " '< join $tiny $coop_lsa' u (\$0):((\$24/(\$46*$i)) / (\$4/(\$16*$i))):( sprintf( '%.2fx',((\$24/(\$46*$i)) / (\$4/(\$16*$i))) ) ):xtic(sprintf(\"%'d (%.2fMB)\",\$1, (\$1*8)/1000000)) t \"LSA CPU-GPU co-op; iGPU shared (CAS compete); Blocks of 5736 on iGPU; sync on block\" lc rgb col_red dt new with linespoints,\\"  >> $CHOICE_TXPS
  ...
  echo  " '< join $tiny $tiny_threads_lsa' u (\$0):((\$20/(\$32*$i)) / (\$4/(\$16*$i))):( sprintf( '%.2fx',((\$20/(\$32*$i)) / (\$4/(\$16*$i))) ) ):xtic(sprintf(\"%'d (%.2fMB)\",\$1, (\$1*8)/1000000)) t \"LSA; TinySTM-wbetl 4 validation worker threads\" lc rgb \"#${gray_palette[2]}\" dt new1 with linespoints,\\"  >> $CHOICE_TXPS
  ##########################
