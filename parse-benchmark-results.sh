...
if [[ "$#" -ne 1 ]];then
    ...
    echo "bash parse-benchmark-results.sh cpu|gpu"
    echo ""
    exit;
fi
...
do
    ...
    do
        ...
        echo "\"STM mode\" \"Validation time (s)\" \"stddev\" \"Commits\" \"stddev\" \"Aborts\" \"stddev\" \"Val Reads\" \"stddev\" \"Val success\" \"stddev\" \"Val fail\" \"stddev\" \"Energy (J)\" \"stddev\" \"Total time (s)\" \"stddev\"" > $f
    done
done
