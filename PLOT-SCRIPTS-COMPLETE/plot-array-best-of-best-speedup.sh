...
echo "set terminal wxt size 2900,1100" > $FILE1
...
echo "set xlabel \"Read-set size\" font\"Computer Modern, 14\"" >> $FILE1
echo "set ylabel \"Program\" font\"Computer Modern, 12\"" >> $FILE1
echo "unset colorbox" >> $FILE1
#echo "unset xtics" >> $FILE1
#echo "set style line 102 lc rgb'#101010' lt 0 lw 4" >> $FILE1

echo "set xtics font \"Computer Modern, 13\"" >> $FILE1
...
for i in 1 2 4 8; do
  for mode in "random"; #"sequential";
    ...
    #untouched
    cpu=$(awk 'NR>8{print $8/$2}' "$RESULTS_DIR/TinySTM-wbetl/$i/array-r99-w1-$mode-walk/1-$mode-cpu-validation")
    #intel
    intel=$(awk 'NR>8{print $12/$2}' "$RESULTS_DIR/TinySTM-igpu-cpu-persistent-dynamic-split-wbetl-block-level-sync-strided-k-4/$i/array-r99-w1-$mode-walk/1-$mode-cpu-validation")
    #amd 
    amd=$(awk 'NR>8{print $12/$2}' "$RESULTS_DIR/TinySTM-igpu-cpu-persistent-dynamic-split-amd-wbetl/$i/array-r99-w1-$mode-walk/1-$mode-cpu-validation")
    #validators
    validators=$(awk 'NR>8{print $8/$2}' "$RESULTS_DIR/TinySTM-threads-wbetl/$i/array-r99-w1-$mode-walk/1-$mode-cpu-validation-4-workers")
    ...
    echo "     '$RESUL_FILE_SPEEDUP' matrix rowheaders columnheaders using 1:2:(((\$3 > 0) ? (sprintf(\"x%.2f\",\$3)) : (sprintf(\"-\")))) with labels font \"Computer Modern,15.5\",\\" >> $FILE1
