...
echo "set terminal wxt size 3200,700" >> $FILE
...
echo "xlabeloffsety=-0.95" >> $FILE
#==========================================================

echo "" >> $FILE
echo "set tics scale 0"  >> $FILE

echo "set xtics nomirror rotate by 45 right scale 0 font \"Computer Modern, 14\""  >> $FILE
...
echo "set key font \"Computer Modern, 15\"" >> $FILE

echo "set ylabel offset -3,0 \"Total reads validated\" font \"Computer Modern, 18\""  >> $FILE
echo "set xlabel offset 0,-3 \"READ SET ENTRIES\" font \"Computer Modern, 18\""  >> $FILE

#echo "set format y \"%2.2f\""  >> $FILE
COUNT=0
#####################################################################################################################################################################
# $2 - validation time, $16 - total time
echo "set title \"Random array walk\" font \"Computer Modern,19\"" >> $FILE
...
#plot errors bars
echo  "      '' u (\$0+0.0055):(1):(sprintf('%.2fx', (\$16>\$14)?((\$16/\$14)):0 )) t \"\" w labels offset char 0.76, char -0.66 font \",13.5\", \\"  >> $FILE
...
#####################################################################################################################################################################
echo "set title \"Sequential array walk\" font \"Computer Modern,16\"" >> $FILE
echo "set logscale y" >> $FILE
echo "plot\\"  >> $FILE
# $14 CPU Val Reads
# $16 GPU Val Reads
...
#draw labels
echo  "      '<tail -n+2 $RESULTS_DIR/TinySTM-igpu-cpu-persistent-dynamic-split-wbetl-block-level-sync-k-1/1/array-r99-w1-sequential-walk/1-sequential-cpu-validation' u (\$0-0.124):((\$14!=0)?(\$14+\$14*0.20):NaN):(sprintf(\"%'d\", \$14)) notitle w labels rotate by 90 left textcolor rgb \"black\" font \"Computer Modern,10\", \\"  >> $FILE
